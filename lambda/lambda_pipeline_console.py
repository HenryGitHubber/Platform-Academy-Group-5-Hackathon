"""
platform-academy-data-pipeline
────────────────────────────────────────────────────────────────────────────
Paste this entire file into the AWS Console inline editor as lambda_function.py

Reads raw files from your S3 bucket:
  employee_data/employee_data.json
  cost_data/cost_data.json
  cost_data/cost_key_data.json
  questionnaire_data/questionnaire_answers.json

Writes to:
  processed/all_data.json  (same bucket)

Environment variable required:
  DATA_BUCKET = platform-academy-raw-data-group5
"""
import csv
import io
import json
...
OUTPUT_KEY_CSV = "processed/all_data.csv"
...
def _read_json(bucket, key):
    try:
        ...
        logger.info("Read s3://%s/%s", bucket, key)
        return json.loads(text)
    except s3.exceptions.NoSuchKey:
        logger.warning("Not found: s3://%s/%s — skipping", bucket, key)
        return None
    except Exception as e:
        logger.error("Error reading s3://%s/%s: %s", bucket, key, e)
        return None


def _write_json(bucket, key, data):
...
def _write_csv(bucket, key, records):
    """Flatten employee records to CSV — one row per employee."""
    if not records:
        return

    # Define flat column order
    fieldnames = [
        "employee_number", "name", "surname", "role", "department_code",
        "years_experience", "stream", "cloud_type", "year_enrolled",
        # training
        "skillbuilder_signed", "skillbuilder_completed",
        "instructor_signed", "instructor_completed",
        "mytms_signed", "mytms_completed",
        "game_day_signed", "game_day_completed",
        "hackathon_signed", "hackathon_completed",
        "received_cert_voucher", "completed_cert_voucher",
        "total_cost_eur",
        # satisfaction
    ...
    for rec in records:
        row = {
            ...
            "year_enrolled":    rec.get("year_enrolled"),
        }
        # Flatten training
        t = rec.get("training", {})
        for field in ["skillbuilder_signed","skillbuilder_completed",
                      "instructor_signed","instructor_completed",
                      "mytms_signed","mytms_completed",
                      "game_day_signed","game_day_completed",
                      "hackathon_signed","hackathon_completed",
        ...
        # Flatten satisfaction
        s = rec.get("satisfaction", {})
        for qid in ["Q1","Q2","Q3","Q4","Q5","Q6","Q7","Q8","Q9","Q10","Q11","Q12"]:
            row[qid] = s.get(qid)
        row["avg_numeric_score"] = s.get("avg_numeric_score")
        writer.writerow(row)
...
def _load_all_data(bucket):
    emp_raw      = _read_json(bucket, "employee_data/employee_data.json")      or {"employees": []}
    cost_raw     = _read_json(bucket, "cost_data/cost_data.json")              or {"costs": []}
    cost_key_raw = _read_json(bucket, "cost_data/cost_key_data.json")          or {"cost_keys": [{}]}
    qa_raw       = _read_json(bucket, "questionnaire_data/questionnaire_answers.json") or []

    employees = list(emp_raw.get("employees", []))
    costs     = list(cost_raw.get("costs", []))
    q_answers = list(qa_raw) if isinstance(qa_raw, list) else []
...
def _calculate_cost(cost_rec, cost_keys):
    total = 0.0
...
# ════════════════════════════════════════════════════════════════════════════
# JOIN: EMPLOYEE + COST + QUESTIONNAIRE → ONE RECORD PER PERSON
# ════════════════════════════════════════════════════════════════════════════
...
def _aggregate_by_year(records):
    ...
    for rec in records:
        ...
        if year not in buckets:
            buckets[year] = {
                "employees": [], "total_cost_eur": 0.0, "q_scores": {},
        ...
        for mod, sk, ck in [
            ...
            b["module_signed"][mod]    += int(t[sk])
            b["module_completed"][mod] += int(t[ck])
...
def _aggregate_by_stream(records):
    ...
    for rec in records:
        ...
        t = rec["training"]
        b["modules_done"]   += sum(int(t[k]) for k in ["skillbuilder_completed","instructor_completed",
                                                         "mytms_completed","game_day_completed","completed_cert_voucher"])
        b["total_modules"]  += 5
    ...
    for stream, b in sorted(buckets.items(), key=lambda x: -x[1]["count"]):
        count = b["count"]
        result.append({
            "stream": stream, "employee_count": count,
            "total_cost_eur":             round(b["total_cost_eur"], 2),
            "avg_cost_per_employee_eur":  round(b["total_cost_eur"]/count, 2) if count else 0,
...
def lambda_handler(event, context):
    ...
    if not bucket:
        raise ValueError("Environment variable DATA_BUCKET is not set")
    ...
    _write_csv(bucket, OUTPUT_KEY_CSV, records)
    logger.info("Pipeline complete — %d employees → s3://%s/%s + .csv", len(records), bucket, OUTPUT_KEY)
